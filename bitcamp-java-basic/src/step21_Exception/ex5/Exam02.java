// 예외 처리 후 
package step21_Exception.ex5;

import java.sql.Date;
import java.util.Scanner;

public class Exam02 {
    // RuntimeException 계열의 예외는 굳이 throws 문장을 선언하지 않아도 되지만, read()를 호출하는 개발자에게 어떤 예외가 발생할 수 있는지 
    // 명확하게 제시해주는 것이 유지보수에 도움이 되기 때문에 메서드 선언부에 발생되는 예외를 명시하는 것이 좋다.

    // => 그런데 조금 아쉬움이 있는 점은 read() 메서드를 사용하는 개발자가 이 메서드에서 RuntimeException을 던진다는 의미에 대해 
    //    직관적으로 이해하기는 어렵다.
