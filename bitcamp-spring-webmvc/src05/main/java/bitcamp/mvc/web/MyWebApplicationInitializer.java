...
public class MyWebApplicationInitializer implements WebApplicationInitializer {
    ...
    public void onStartup(ServletContext servletContext) throws ServletException {
        //웹 어플리케이션을 시작할 때 이 메서드를 호출 한다.
        //누가 실행하지??
        //ServletContainer(ex; Tomcat)
        //      ====> spring-web.jar -> /META-INF/services/javax.servletServletContainerInitializer 파일
        //          ====> 파일에서 구현체 정보를 알아내어 객체를 생성한 후 호출
        //          ====> SpringServletContainerInitializer.onStartup() 호출
        //              ====> WebApplicationInitializer 구현체의 onStartup()을 호출
        //              ====> MyWebApplicationInitializer.onStartup() 호출
        System.out.println("===> MyWebApplicationInitializer.onStartup()");
        
        //여기에서 프런트 컨트롤러를 등록할 것이다.
        //1) 프론트 컨트롤러가 사용할 Spring IOC Container를 만든다.
        ...
        //3) 프론트 컨트롤러를 웹 어플리케이션에 등록한다.
        //  addServlet()의 리턴값은  서블릿 url-pattern등을 설정하는 도구
        Dynamic registration = servletContext.addServlet("mvc", mvcDispatcher);
        
        //4) load-on-startup값 설정
