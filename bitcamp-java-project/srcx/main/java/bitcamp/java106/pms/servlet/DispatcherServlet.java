...
// 하는 일:
//  => 클라이언트의 요청이 들어오면 그 요청을 컨트롤러에게 위임하는 역할
...
public class DispatcherServlet implements Servlet{
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        // 그래서 서블릿이 실행하는데 필요한 자원을 준비하는 코드를 여기에 작성하면 된다.
        this.config = config;
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        // 클라리언트 요청이 들어올 때 마다 서블릿 컨테이너가 호출하는 메서드
        //  => 여기에 클라리언트의 요청을 처리하는 코드를 두면 된다.
    ...
    public String getServletInfo() {
        ...
        return "요청 처리를 중계하는 DispatcherSevelet";
    }

    @Override
    public void destroy() {
        // 서버를 종료하기 전에 서블릿 컨테이너가 호출한다.
        //  => 여기에는 서블릿이 실행되는 동안 만들었던 자원을 해제하는 코드를 두면된다.
...
//      => $톰캣홈/webapp/pms/WEB-INF/classes 폴더생성
...
//      이클립스에서 컴파일하거나 실행할 때 사용하는 외부 라이브버리는 gradle이 관리하는 라이브러리이다.
