...
public class TaskController implements Controller {
    ...
    private void onTaskAdd(final Team team) {
        ...
        if (str.length() == 0) {
            ...
            if (data.getTime() < team.getStartDate().getTime()) {
                task.setStartDate(team.getStartDate());
            } else {
        ...
        if (str.length() == 0) {
            ...
            Date data = Date.valueOf(str);
            if (data.getTime() > team.getEndDate().getTime()) {
                task.setEndDate(team.getEndDate());
            } else {
                task.setEndDate(data);
            }
        }
    ...
    private void onTaskList(final Team team) {
        ...
        while (iterator.hasNext()) {
            Task oritask = iterator.next();
            System.out.printf("%d,%s,%s,%s,%s\n", 
                    oritask.getNo(), oritask.getTitle(), 
                    oritask.getStartDate(), oritask.getEndDate(),
                    (oritask.getWorker() == null) ? 
                            "-" : oritask.getWorker().getId());
        }
        System.out.println();
    }
    ...
    private void onTaskUpdate(final Team team) {
        ...
        if (Console.confirm("변경하시겠습니까?")) {
            int index = taskDao.indexOf(memberId);
