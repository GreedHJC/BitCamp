...
## Src13 - 날짜 값을 다루는 java.sql.Date 클래스 적용(리팩토링)
 ...
 - 작업내용
   - 팀, 회원, 게시물 정보 입력/변경과 학원 정보 입력/변경 시 날짜 값을 Date 객체에 저장한다.
...
## Src12 - 생성자 도입( 리팩토링)
 - 학습목표
   ...
   - 생성자 문법의 존재 의미를 이해한다.
 - 작업내용
   -XxxController 객체를 생성 할때 필수 값으로 Scanner 객체를 받도록 한다.

## Src11 - 인스턴스 변수와 인스턴스 메서드의 활용(리펙토링)
 - 학습 목표
   - 인스턴스 변수의 사용법을 익힌다.
   - 인스턴스 메서드의 사용법을 익힌다.
 - 작업내용
   - 스테틱 변수와 스테틱 메서드로 만든 기존 코드를 인스턴스 변수와 인스턴스 메소드로 변경한다.
...
- 실행 결과
```
>javac -encoding UTF-8 -d bin -sourcepath src/main/java src/main/java/bitcamp/java106/pms/App.java-X
>javac -encoding UTF-8 -d bin src/main/java/bitcamp/java106/pms/App.java
> java -classpath bin bitcamp.java106.pms.App
> java -cp bin main.java.bitcamp.java106.pms.App
팀명? 비트비트
설명? 자바 프로젝트 팀
최대인원? 5
시작일? 2018-05-05
