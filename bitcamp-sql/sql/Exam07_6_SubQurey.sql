...
/* => 1단계 */
select la.lano, la.lno, la.mno, la.rdt from lect_appl la; 
...
/* from 절에서 반복적으로 사용하는 서브쿼리가 있다면, 차라리 가상테이블인 view로 정의해놓고 사용하는 것이 편하다. */
...
from lect_appl la 
                  join lect2 as lec on la.lno=lec.lno
      where lec.manager_no in (select mno from mgr where posi in ('과장', '대리'));
